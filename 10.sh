...
Insert()
{
		...
		no_of_digits=`echo -n $guest_phone | wc -c`	
                test $no_of_digits -le 9 && field_error=2
		test $no_of_digits -ge 11 && field_error=2 #if phone is not 10 digit
		if [ $field_error -eq 2 ]
		then 
			echo "this is field error in IF "
		fi
	    else
		echo "this is field_error in ELSE "
		field_error=2
            fi
		...
		field_error=21
            ...
            case $field_error in
                    ...
                    0)
                             echo -e "\n$gno,$guest_name,$guest_phone,$guest_count,$guest_suite,$guest_room,$guest_check_in,$guest_check_out,true" >> Guest.txt
